...
import { supabase, isAllowedEmail } from './supabaseClient';
import logo from './assets/logo.png';

const APP_DATA = [
  { id: 1, name: "Room Scheduler", cat: "Internal", desc: "Schedule smarter and Book faster. Manage meeting rooms and check live availability.", url: "https://stlaf-room-scheduler.vercel.app", dept: "IT Department" },
  { id: 2, name: "Liaison Operations Tracker", cat: "Operations", desc: "Centralized platform for assigning, monitoring, and managing field liaison tasks across all firm departments.", url: "https://stlaf-lo-tracker.base44.app/", dept: "Operation Department" },
  { id: 3, name: "IT Asset Management", cat: "IT Management", desc: "Track, manage, and optimize the lifecycle of hardware assets.", url: "https://stlaf-it-asset-management-system.vercel.app", dept: "IT Department" },
  { id: 4, name: "Ticketing System", cat: "IT Support", desc: "Allows users to submit and track tickets for hardware and software issues.", url: "http://192.168.112.12:3000/", dept: "IT Department" },
  { id: 5, name: "IT Support Bot", cat: "Knowledge Assistant", desc: "Uses AI to analyze documents, answer questions, and generate insights based on uploaded IT-related materials.", url: "https://notebooklm.google.com/notebook/91564952-5aa6-4032-897b-56a0146f1f3a", dept: "IT Department" },
  { id: 6, name: "Leave and Overtime System", cat: "Internal", desc: "A web application designed for tracking employee time, managing overtime hours, and requesting leave.", url: "https://stlaf-leave.vercel.app", dept: "HR Department" },
  { id: 7, name: "HR Portal", cat: "Internal", desc: "An interactive HR dashboard for tracking personnel onboarding and offboarding workflows, synced with Google Sheets data.", url: "https://stlaf-hr-portal.vercel.app", dept: "HR Department" },
  { id: 8, name: "Property Tax Calculator", cat: "Corporate", desc: "An application for property tax calculator used for calculation and real-life tax simulation.", url: "https://stlaf-corpo-tax-calc.vercel.app/Calculator", dept: "Corporate Department" },
  { id: 9, name: "Corporate Drafting", cat: "Corporate", desc: "An application for drafting variety of legal documents like SPA and SEC certificate.", url: "https://stlaf-corpo-drafting.vercel.app", dept: "Corporate Department" },
  { id: 10, name: "Amendment Engine", cat: "Corporate", desc: "An application used for generating new Amendment for corporations and partnerships.", url: "https://stlaf-corpo-amendment-generator.vercel.app/", dept: "Corporate Department" },
  { id: 11, name: "Operations Management", cat: "Corporate", desc: "An application used for tracking and managing operational transactions.", url: "https://stlaf-corpo-ops-monitoring.vercel.app", dept: "Corporate Department" },
  { id: 12, name: "STLAF Marketing Content Planner ", cat: "Content Planner", desc: "This web application is a unified content marketing dashboard that allows you to schedule and publish posts directly to Facebook or Instagram while seamlessly bridges authorized campaign updates into direct email newsletter broadcasts.", url: "https://stlaf-marketing-planner.vercel.app", dept: "Marketing Department" },
  { id: 13, name: "STLAF Marketing Newsletter", cat: "Marketing", desc: "This web application is an email marketing and newsletter platform that allows you to manage subscriber lists, design email templates, and schedule or send automated bulk campaigns directly through your connected Gmail account.", url: "https://stlaf-marketing-newsletter.vercel.app", dept: "Marketing Department" },
  { id: 14, name: "STLAPPss-Litigation", cat: "Internal", desc: "Need anything for work? Check this app.", url: "https://sites.google.com/view/stlaf-litigation/home", dept: "Litigation Department" },
  { id: 15, name: "Vacant", cat: "Internal", desc: "Need anything for work? Check this app.", url: "https://sites.google.com/view/stlaf-litigation/home", dept: "Litigation Department" },
];
...
function App() {
    ...
    const handleSession = async (session) => {
      if (session) {
        ...
        if (userEmail && !isAllowedEmail(userEmail)) {
          ...
          setTimeout(() => setLoginCardVisible(true), 100);
          setLoginError("Access restricted to STLAF accounts only.");
          setLoginShake(true);
  ...
  useEffect(() => {
    ...
    const savedEmail = localStorage.getItem("stlaf_remember_email");
  ...
  useEffect(() => { const h = (e) => { if (e.key === "Escape") { if (selectedApp) handleClosePanel(); setUserMenuOpen(false); } }; document.addEventListener("keydown", h); return () => document.removeEventListener("keydown", h); }, [selectedApp]);
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (error) { setLoginError(error.message); setLoginShake(true); setTimeout(() => setLoginShake(false), 600); }
      else {
        if (rememberMe) localStorage.setItem("stlaf_remember_email", email);
        else localStorage.removeItem("stlaf_remember_email");
        setSession(data.session); setIsLoggedIn(true); setCurrentPage("apps"); setShowLoginPanel(false); setLoginCardVisible(false); setLoginError("");
      }
    } catch { setLoginError("An unexpected error occurred."); setLoginShake(true); setTimeout(() => setLoginShake(false), 600); }
    setIsLoading(false);
  };
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error) { setLoginError(error.message); setLoginShake(true); setTimeout(() => setLoginShake(false), 600); setGoogleLoading(false); }
    } catch { setLoginError("Failed to connect to Google."); setLoginShake(true); setTimeout(() => setLoginShake(false), 600); setGoogleLoading(false); }
  };

  const handleLogout = async () => {
    await supabase.auth.signOut();
    setIsLoggedIn(false); setSession(null); setCurrentPage("welcome"); setShowLoginPanel(false); setLoginCardVisible(false);
    setSearchTerm(""); setActiveFilter("All"); setSelectedApp(null); setUserMenuOpen(false);
  ...
  const handleLaunchClick = (url) => {
    ...
    setIsLaunching(true); setLaunchProgress(0); launchUrlRef.current = url;
    let progress = 0; const totalDuration = 1200; const interval = 16; const increment = 100 / (totalDuration / interval);
    launchTimerRef.current = setInterval(() => {
      progress += increment;
      if (progress >= 100) { progress = 100; clearInterval(launchTimerRef.current); setLaunchProgress(100); setTimeout(() => { window.open(launchUrlRef.current, "_blank", "noopener,noreferrer"); setTimeout(() => { setIsLaunching(false); setLaunchProgress(0); }, 400); }, 200); }
      else { setLaunchProgress(progress); }
    }, interval);
  };
  ...
  const getUserDisplayName = () => {
    ...
    return emailStr.split("@")[0].replace(/\./g, " ").replace(/\b\w/g, c => c.toUpperCase());
  };
  ...
  const renderPanelContent = (app, tab = null) => {
    ...
    return (<>
      {showInfo && (<>
        <div className="flex items-center gap-2 mb-5 pb-4" style={{ borderBottom: `1px solid ${dm ? n.borderPanel : "rgba(255,255,255,.1)"}` }}><svg className="w-4 h-4" style={{ color: n.gold }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg><p className="text-xs font-bold uppercase tracking-widest" style={{ color: n.gold }}>{app.dept}</p></div>
        {tab === null && (<><h4 className="text-[11px] font-bold uppercase tracking-widest mb-3" style={{ color: dm ? n.neonMuted : "rgba(255,255,255,.3)" }}>About</h4><p className="text-sm font-light leading-relaxed mb-7" style={{ color: dm ? n.textSecondary : "rgba(255,255,255,.6)" }}>{app.desc}</p></>)}
        ...
        <div className="rounded-2xl overflow-hidden" style={{ border: `1px solid ${dm ? n.borderPanel : "rgba(255,255,255,.08)"}` }}>
          {[{ label: "Category", value: app.cat }, { label: "Department", value: app.dept }, { label: "Status", isStatus: true }].map((row, i) => (
            <div key={i} className="flex items-center justify-between px-4 py-3" style={{ borderBottom: i < 2 ? `1px solid ${dm ? n.borderPanel : "rgba(255,255,255,.08)"}` : "none", background: i % 2 === 0 ? (dm ? n.bgPanelSection : "rgba(255,255,255,.03)") : "transparent" }}>
              ...
              {row.isStatus ? <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full" style={{ background: dm ? n.green : "#4ade80", boxShadow: `0 0 6px ${dm ? n.greenDim : "rgba(74,222,128,.5)"}` }} /><span className="text-xs font-semibold" style={{ color: dm ? n.green : "#4ade80" }}>Active</span></span> : <span className="text-xs font-semibold" style={{ color: dm ? n.textPrimary : "rgba(255,255,255,.8)" }}>{row.value}</span>}
            </div>))}
        </div>
        {tab === null && (<div className="mt-5 rounded-xl p-4" style={{ background: dm ? n.bgPanelSection : "rgba(255,255,255,.05)", border: `1px solid ${dm ? n.borderPanel : "rgba(255,255,255,.08)"}` }}><div className="flex items-center gap-2 mb-2"><svg className="w-3.5 h-3.5" style={{ color: dm ? n.neonMuted : "rgba(255,255,255,.5)" }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg><p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: dm ? n.textDim : "rgba(255,255,255,.4)" }}>URL</p></div><p className="text-xs font-medium break-all" style={{ color: dm ? n.textMuted : "rgba(255,255,255,.5)" }}>{app.url}</p></div>)}
      </>)}
      {tab === "about" && (<><h4 className="text-lg font-black tracking-tight mb-4" style={{ color: dm ? n.textPrimary : "white" }}>{app.name}</h4><p className="text-sm font-light leading-relaxed mb-6" style={{ color: dm ? n.textSecondary : "rgba(255,255,255,.6)" }}>{app.desc}</p><div className="rounded-xl p-4 mb-4" style={{ background: dm ? n.bgPanelSection : "rgba(255,255,255,.05)", border: `1px solid ${dm ? n.borderPanel : "rgba(255,255,255,.08)"}` }}><div className="flex items-center gap-2 mb-2"><svg className="w-3.5 h-3.5" style={{ color: n.gold }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5" /></svg><p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: n.gold }}>Department</p></div><p className="text-sm font-medium" style={{ color: dm ? n.textPrimary : "rgba(255,255,255,.7)" }}>{app.dept}</p></div><div className="rounded-xl p-4" style={{ background: dm ? n.bgPanelSection : "rgba(255,255,255,.05)", border: `1px solid ${dm ? n.borderPanel : "rgba(255,255,255,.08)"}` }}><div className="flex items-center gap-2 mb-2"><svg className="w-3.5 h-3.5" style={{ color: dm ? n.neonMuted : "rgba(255,255,255,.5)" }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg><p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: dm ? n.textDim : "rgba(255,255,255,.4)" }}>URL</p></div><p className="text-xs font-medium break-all" style={{ color: dm ? n.textMuted : "rgba(255,255,255,.5)" }}>{app.url}</p></div></>)}
    </>);
  };

  const LaunchButton = ({ url }) => {
    const done = launchProgress >= 100; const active = isLaunching; const pct = Math.min(launchProgress, 100);
    const baseColor = dm ? n.bgPanelSection : "white"; const textIdle = dm ? n.textPrimary : "#123765";
    const currentText = done ? "Launching..." : active ? `${Math.round(pct)}%` : "Launch App";
    const currentTextColor = active ? (pct > 50 ? "white" : textIdle) : textIdle;
    return (
      <button onClick={(e) => { e.preventDefault(); handleLaunchClick(url); }} disabled={isLaunching}
        className="launch-btn relative block w-full py-3.5 rounded-xl font-bold text-sm text-center overflow-hidden select-none"
        style={{ background: baseColor, color: done ? "white" : currentTextColor, border: dm ? `1px solid ${n.borderPanel}` : "none", boxShadow: active ? (dm ? `0 0 20px ${n.gold}33` : `0 8px 25px ${n.gold}33`) : (dm ? `0 0 10px ${n.neonGhost}` : "0 4px 14px rgba(0,0,0,.08)"), cursor: isLaunching ? "not-allowed" : "pointer", transition: "box-shadow .3s ease, color .15s ease" }}>
        <div style={{ position: "absolute", top: 0, left: 0, height: "100%", width: `${pct}%`, background: n.gold, borderRadius: "inherit", transition: pct === 0 ? "none" : "width 0.05s linear", zIndex: 0 }} />
        {active && !done && <div className="launch-shimmer" style={{ position: "absolute", top: 0, left: 0, height: "100%", width: `${pct}%`, background: "linear-gradient(90deg, transparent 0%, rgba(255,255,255,.25) 50%, transparent 100%)", backgroundSize: "200% 100%", borderRadius: "inherit", zIndex: 1 }} />}
        <span className="relative z-10 flex items-center justify-center gap-2">
          {done && <svg className="w-4 h-4 launch-check" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>}
          {active && !done && <svg className="w-4 h-4 launch-spinner" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>}
  ...
  if (authLoading) {
    return (<div className="h-screen flex items-center justify-center bg-[#0A3055]"><div className="text-center"><img src={logo} alt="STLAF" className="h-16 w-auto object-contain mx-auto mb-4 opacity-80" /><div className="flex items-center justify-center gap-2 text-white/50"><svg className="w-5 h-5 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg><span className="text-sm font-medium">Loading...</span></div></div></div>);
  }
  ...
  return (
    <div className="flex flex-col min-h-screen font-sans text-[#0A3055]">
      <style>{`
        ...
        .detail-panel-in{animation:panelSlideIn .4s cubic-bezier(.22,1,.36,1) both}.detail-panel-out{animation:panelSlideOut .3s cubic-bezier(.4,0,1,1) both}
        .mobile-panel-in{animation:panelMobileIn .4s cubic-bezier(.22,1,.36,1) both}.mobile-panel-out{animation:panelMobileOut .3s cubic-bezier(.4,0,1,1) both}
        .login-shake{animation:shake .6s ease-in-out}.spinner{animation:spin 1s linear infinite}.page-enter{animation:fadeIn .5s ease-out}
        ...
        .no-scroll{height:100vh;overflow:hidden}.close-btn{transition:all .3s ease}.close-btn:hover{transform:rotate(90deg) scale(1.1)}
        ...
        .login-overlay{position:fixed;inset:0;z-index:100;display:flex;align-items:center;justify-content:center;padding:1rem;pointer-events:none;opacity:0;transition:opacity .3s ease}.login-overlay.visible{pointer-events:all;opacity:1}
        .login-card-animated{transform:translateY(40px) scale(.95);opacity:0;transition:all .5s cubic-bezier(.34,1.56,.64,1)}.login-card-animated.show{transform:translateY(0) scale(1);opacity:1}.login-card-animated.closing{transform:translateY(30px) scale(.97);opacity:0;transition:all .35s cubic-bezier(.4,0,1,1)}
        ...
        .list-scrollbar::-webkit-scrollbar{width:4px}.list-scrollbar::-webkit-scrollbar-track{background:transparent}.list-scrollbar::-webkit-scrollbar-thumb{background:rgba(0,191,255,.2);border-radius:10px}.list-scrollbar::-webkit-scrollbar-thumb:hover{background:rgba(0,191,255,.4)}.list-scrollbar{scrollbar-width:thin;scrollbar-color:rgba(0,191,255,.2) transparent}
        .list-scrollbar-light::-webkit-scrollbar{width:4px}.list-scrollbar-light::-webkit-scrollbar-track{background:transparent}.list-scrollbar-light::-webkit-scrollbar-thumb{background:rgba(150,150,150,.3);border-radius:10px}.list-scrollbar-light::-webkit-scrollbar-thumb:hover{background:rgba(150,150,150,.5)}.list-scrollbar-light{scrollbar-width:thin;scrollbar-color:rgba(150,150,150,.3) transparent}
        .hide-scrollbar::-webkit-scrollbar{display:none}.hide-scrollbar{-ms-overflow-style:none;scrollbar-width:none;overflow-y:auto}
        .launch-btn{position:relative;overflow:hidden}.launch-shimmer{animation:launchShimmer 1s linear infinite}.launch-spinner{animation:launchSpin .8s linear infinite}.launch-check{animation:launchCheckPop .4s cubic-bezier(.34,1.56,.64,1) both}
        .google-btn{transition:all .3s ease}.google-btn:hover{transform:translateY(-1px);box-shadow:0 4px 12px rgba(0,0,0,.15)!important}.google-btn:active{transform:scale(.98)}
        .user-menu-enter{animation:dropdownIn .2s ease-out both}
        @keyframes shine {0% {background-position: 200% center;}100% {background-position: -200% center;}}
        .shine-text {background: linear-gradient(90deg,#0a3055 0%,#ccaa49 50%,#0a3055 100%);background-size: 200% auto;background-clip: text;-webkit-background-clip: text;color: transparent;animation: shine 4s linear infinite;}
              ...
              <div className="login-split-card bg-white rounded-3xl shadow-2xl shadow-black/20 overflow-hidden flex flex-col md:flex-row min-h-[520px]">
                ...
                <div className="login-right-panel md:w-[55%] bg-white p-3 md:p-4">
                  <div className="login-right-inner w-full h-full bg-[#123765] rounded-3xl p-8 md:p-10 flex flex-col justify-center items-center relative border border-white/40">
                    ...
                    <div className="md:hidden mb-6"><img src={logo} alt="STLAF" className="h-10 w-auto object-contain mx-auto brightness-0 invert opacity-80" /></div>
                    <div className="slide-up w-full text-center"><h3 className="text-white text-2xl md:text-3xl font-black tracking-tight mb-1">Welcome</h3><p className="text-white/50 text-sm font-light mb-8">Continue with your STLAF Google account</p></div>
                    ...
                    <div className="w-full slide-up-delay"><button onClick={() => setShowEmailForm(!showEmailForm)} className="w-full flex items-center gap-3 py-3 group"><div className="flex-grow h-px bg-white/10" /><span className="text-[10px] text-white/30 font-semibold uppercase tracking-widest group-hover:text-white/50 transition-colors">{showEmailForm ? "Hide" : "Use email instead"}</span><div className="flex-grow h-px bg-white/10" /></button></div>
                    {showEmailForm && (
                      <form onSubmit={handleLogin} className="space-y-4 w-full mt-2">
                        <div className="slide-up"><label className="block text-[11px] font-semibold uppercase tracking-wider text-white/50 mb-2">Email</label><div className="relative"><div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none"><svg className="w-[18px] h-[18px] text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg></div><input type="email" value={email} onChange={(e) => { setEmail(e.target.value); setLoginError(""); }} placeholder="stlaf.name@gmail.com" required disabled={isLoading || googleLoading} className="input-field w-full pl-12 pr-4 py-3.5 bg-white rounded-xl text-sm font-medium text-[#0A3055] outline-none disabled:opacity-50 placeholder:text-gray-400 placeholder:font-light" /></div></div>
                        <div className="slide-up"><label className="block text-[11px] font-semibold uppercase tracking-wider text-white/50 mb-2">Password</label><div className="relative"><div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none"><svg className="w-[18px] h-[18px] text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg></div><input type={showPassword ? "text" : "password"} value={password} onChange={(e) => { setPassword(e.target.value); setLoginError(""); }} placeholder="Enter your password" required disabled={isLoading || googleLoading} className="input-field w-full pl-12 pr-12 py-3.5 bg-white rounded-xl text-sm font-medium text-[#0A3055] outline-none disabled:opacity-50 placeholder:text-gray-400 placeholder:font-light" /><button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400 hover:text-[#0A3055] transition-colors"><svg className="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">{showPassword ? <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.878 9.878L3 3m6.878 6.878L21 21" /> : <><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></>}</svg></button></div></div>
                        <div className="flex items-center gap-3"><input type="checkbox" id="rememberMe" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} className="checkbox-custom" disabled={isLoading} /><label htmlFor="rememberMe" className="text-sm text-white/50 font-light cursor-pointer select-none">Remember me</label></div>
                        <div className="pt-1"><button type="submit" disabled={isLoading || googleLoading || !email || !password} className="w-full bg-[#0A3055] text-white py-3.5 rounded-xl font-bold text-sm tracking-wide shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2">{isLoading ? <><svg className="w-4 h-4 spinner" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>Signing in...</> : "Sign in with Email"}</button></div>
                      </form>
                    )}
                    {loginError && <div className="w-full mt-4 flex items-center gap-2.5 bg-red-50 border border-red-100 text-red-500 px-4 py-3 rounded-xl text-xs font-medium"><svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>{loginError}</div>}
        ...
        <div className="h-screen flex flex-col page-enter transition-colors duration-300 overflow-hidden" style={{ background: dm ? n.bg : "#f0f4f8", color: dm ? n.textPrimary : "#0A3055" }}>
                    ...
                    {userMenuOpen && (
                      <div className="user-menu-enter absolute right-0 top-full mt-2 w-56 rounded-xl shadow-2xl border overflow-hidden z-50"
                        ...
                        <button
                          ...
                          style={{ color: dm ? "#f87171" : "#ef4444" }}
                          onMouseEnter={(e) => { e.currentTarget.style.background = dm ? "rgba(248,113,113,.08)" : "#fef2f2"; }}
          ...
          <div className="shrink-0 z-40 transition-colors duration-300" style={{ background: dm ? n.bg : "#f0f4f8", borderBottom: `1px solid ${dm ? n.neonGhost : "rgba(229,231,235,.5)"}` }}>
            <div className="max-w-[1600px] mx-auto px-4 md:px-10 py-3"><div className="flex items-center justify-between gap-4">
              <div className="hidden md:flex rounded-xl p-1 items-center gap-0.5 overflow-x-auto filter-scroll flex-grow" style={{ background: dm ? n.neonGhost : "white", border: `1px solid ${dm ? n.border : "#e5e7eb"}` }}>{departments.map(dept => (<button key={dept} onClick={() => setActiveFilter(dept)} className="px-3.5 py-1.5 rounded-lg text-[11px] font-semibold tracking-wide whitespace-nowrap transition-all" style={activeFilter === dept ? { background: dm ? n.neon : "#0A3055", color: dm ? n.bg : "white", boxShadow: dm ? `0 0 12px ${n.neonDim}` : "0 1px 3px rgba(0,0,0,.1)" } : { color: dm ? n.textMuted : "#6b7280" }}>{dept}</button>))}</div>
              <div className="md:hidden relative flex-grow" ref={dropdownRef}><button onClick={() => setDropdownOpen(!dropdownOpen)} className="w-full rounded-xl border px-4 py-2.5 flex items-center justify-between text-sm font-semibold" style={{ background: dm ? n.neonGhost : "white", borderColor: dm ? n.border : "#e5e7eb", color: dm ? n.textPrimary : "#0A3055" }}><span>{activeFilter === "All" ? "All Departments" : activeFilter}</span><svg className={`w-4 h-4 transition-transform ${dropdownOpen ? "rotate-180" : ""}`} style={{ color: dm ? n.textDim : "#9ca3af" }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></button>{dropdownOpen && (<div className="dropdown-enter absolute top-full left-0 right-0 mt-2 rounded-xl shadow-xl py-1 z-50 max-h-[280px] overflow-y-auto" style={{ background: dm ? n.bgCard : "white", border: `1px solid ${dm ? n.border : "#e5e7eb"}` }}>{departments.map(dept => (<button key={dept} onClick={() => { setActiveFilter(dept); setDropdownOpen(false); }} className="w-full text-left px-4 py-2.5 text-sm font-medium transition-colors" style={{ color: activeFilter === dept ? (dm ? n.neon : "#0A3055") : (dm ? n.textMuted : "#6b7280") }}>{dept === "All" ? "All Departments" : dept}</button>))}</div>)}</div>
            ...
            <div className="max-w-[1600px] mx-auto px-4 md:px-10 h-full">
              <div className="hidden md:flex gap-5 h-full py-4">
                <div ref={listRef} className={`overflow-y-auto pr-2 transition-all duration-[450ms] ease-in-out ${dm ? "list-scrollbar" : "list-scrollbar-light"} ${panelOpen ? "w-[340px] lg:w-[400px] flex-shrink-0" : "w-full"}`}>
                  <div className={`${panelOpen ? "flex flex-col gap-2.5" : "grid grid-cols-2 lg:grid-cols-3 gap-4"} pb-4`}>
                    {filteredApps.map((app, i) => {
                      ...
                      if (panelOpen) return (<div key={app.id} onClick={() => handleSelectApp(app)} className="app-list-item cursor-pointer rounded-xl border p-4 flex items-center gap-4" style={{ borderColor: sel ? (dm ? n.borderActive : "rgba(10,48,85,.25)") : (dm ? n.neonGhost : "#e5e7eb"), background: sel ? (dm ? `${n.neon}10` : "rgba(10,48,85,.04)") : (dm ? n.bgCard : "white") }}><div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 text-sm font-black" style={{ background: sel ? (dm ? n.neon : "#0A3055") : (dm ? n.neonFaint : "rgba(10,48,85,.1)"), color: sel ? (dm ? n.bg : "white") : (dm ? n.neonSoft : "rgba(10,48,85,.6)") }}>{app.name.charAt(0)}</div><div className="flex-grow min-w-0"><h3 className="text-sm font-bold tracking-tight truncate" style={{ color: dm ? n.textPrimary : "#0A3055" }}>{app.name}</h3><p className="text-[10px] font-medium uppercase tracking-widest truncate" style={{ color: dm ? n.neonMuted : n.gold }}>{app.dept}</p></div><div className="flex items-center gap-1.5 flex-shrink-0"><span className="w-1.5 h-1.5 rounded-full" style={{ background: dm ? n.green : "#22c55e" }} /><svg className="w-3.5 h-3.5" style={{ color: sel ? (dm ? n.neon : "#0A3055") : (dm ? n.neonFaint : "#d1d5db") }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg></div></div>);
                      return (<div key={app.id} onClick={() => handleSelectApp(app)} className={`app-card ${dm ? "app-card-dark" : "app-card-light"} card-pop cursor-pointer group rounded-2xl border p-5 sm:p-6`} style={{ borderColor: dm ? n.neonGhost : "#e5e7eb", background: dm ? n.bgCard : "white" }} onMouseEnter={dm ? (e) => { e.currentTarget.style.background = n.bgCardHover; } : undefined} onMouseLeave={dm ? (e) => { e.currentTarget.style.background = n.bgCard; } : undefined}><div className="flex items-center justify-between mb-4"><span className="text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded" style={{ background: dm ? n.neonGhost : "#eff6ff", color: dm ? n.neonSoft : "#60a5fa" }}>{app.cat}</span><div className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full" style={{ background: dm ? n.green : "#22c55e" }} /><span className="text-[9px] font-medium" style={{ color: dm ? n.greenDim : "rgba(22,163,74,.6)" }}>Active</span></div></div><h3 className="text-base sm:text-lg font-bold tracking-tight leading-snug mb-1.5 transition-colors duration-200" style={{ color: dm ? n.textPrimary : "#0A3055" }}>{app.name}</h3><p className="text-[10px] font-semibold uppercase tracking-widest mb-4" style={{ color: dm ? n.neonMuted : n.gold }}>{app.dept}</p><div className="flex items-center justify-between pt-4 border-t" style={{ borderColor: dm ? n.neonGhost : "#f3f4f6" }}><span className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: dm ? n.textDim : "#d1d5db" }}>View details</span><div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: dm ? n.neonGhost : "#f3f4f6", color: dm ? n.textDim : "#d1d5db" }}><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg></div></div></div>);
                    })}
                  </div>
                ...
                {selectedApp && (
                  <div ref={detailRef} className={`flex-grow min-w-0 rounded-2xl flex flex-col overflow-hidden ${panelClosing ? "detail-panel-out" : "detail-panel-in"}`} style={{ background: dm ? n.bgPanel : "#123765", border: `1px solid ${dm ? n.borderPanel : "rgba(255,255,255,.1)"}`, boxShadow: dm ? `0 0 50px ${n.neonGhost}, 0 20px 60px rgba(0,0,0,.3)` : "0 20px 60px rgba(10,48,85,.25)" }}>
                    <div className="px-6 pt-6 pb-4 flex-shrink-0" style={{ borderBottom: `1px solid ${dm ? n.borderPanel : "rgba(255,255,255,.1)"}` }}><div className="flex items-start justify-between mb-3"><div className="flex items-center gap-2"><span className="text-[9px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-lg" style={{ background: dm ? n.neonFaint : "rgba(255,255,255,.15)", color: dm ? n.neon : "rgba(255,255,255,.8)" }}>{selectedApp.cat}</span><span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full" style={{ background: dm ? n.green : "#4ade80", boxShadow: `0 0 6px ${dm ? n.greenDim : "rgba(74,222,128,.5)"}` }} /><span className="text-[9px] font-semibold" style={{ color: dm ? n.green : "#4ade80" }}>Active</span></span></div><button onClick={handleClosePanel} className="close-btn w-8 h-8 flex items-center justify-center rounded-xl transition-all" style={{ background: dm ? n.neonFaint : "rgba(255,255,255,.1)", color: dm ? n.textMuted : "rgba(255,255,255,.5)" }}><svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button></div><h2 className="text-xl font-black tracking-tight leading-tight" style={{ color: dm ? n.textPrimary : "white" }}>{selectedApp.name}</h2></div>
                    <div className="flex-grow hide-scrollbar p-6">{renderPanelContent(selectedApp)}</div>
                    <div className="flex-shrink-0 px-6 pb-6 pt-3" style={{ borderTop: `1px solid ${dm ? n.borderPanel : "rgba(255,255,255,.1)"}` }}><LaunchButton url={selectedApp.url} /></div>
                  </div>
                )}
              </div>
              ...
              <div className="md:hidden h-full overflow-y-auto pb-4 pt-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pb-4">
                  {filteredApps.map((app) => (<div key={app.id} onClick={() => handleSelectApp(app)} className={`app-card ${dm ? "app-card-dark" : "app-card-light"} card-pop cursor-pointer rounded-2xl border p-5`} style={{ borderColor: selectedApp?.id === app.id ? (dm ? n.borderActive : "rgba(10,48,85,.2)") : (dm ? n.neonGhost : "#e5e7eb"), background: selectedApp?.id === app.id ? (dm ? `${n.neon}08` : "rgba(10,48,85,.02)") : (dm ? n.bgCard : "white") }}><div className="flex items-center justify-between mb-3"><span className="text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded" style={{ background: dm ? n.neonGhost : "#eff6ff", color: dm ? n.neonSoft : "#60a5fa" }}>{app.cat}</span><span className="w-1.5 h-1.5 rounded-full" style={{ background: dm ? n.green : "#22c55e" }} /></div><h3 className="text-sm font-bold tracking-tight mb-1" style={{ color: dm ? n.textPrimary : "#0A3055" }}>{app.name}</h3><p className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: dm ? n.neonMuted : n.gold }}>{app.dept}</p></div>))}
                </div>
                {filteredApps.length === 0 && (<div className="text-center py-20"><p className="text-lg font-black tracking-tight mb-2" style={{ color: dm ? n.textPrimary : "#0A3055" }}>No apps found</p><p className="text-xs font-light mb-5" style={{ color: dm ? n.textDim : "#9ca3af" }}>Try adjusting your search or filter</p><button onClick={() => { setActiveFilter("All"); setSearchTerm(""); }} className="px-5 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all" style={{ background: dm ? n.neon : "#0A3055", color: dm ? n.bg : "white" }}>Reset</button></div>)}
                {selectedApp && (
                  <div className={`fixed inset-0 z-[70] flex flex-col ${panelClosing ? "mobile-panel-out" : "mobile-panel-in"}`} style={{ background: dm ? n.bgPanel : "#123765" }}>
                    <div className="px-5 pt-5 pb-3 flex-shrink-0" style={{ borderBottom: `1px solid ${dm ? n.borderPanel : "rgba(255,255,255,.1)"}` }}><div className="flex items-start justify-between mb-3"><div className="flex items-center gap-2"><span className="text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded-lg" style={{ background: dm ? n.neonFaint : "rgba(255,255,255,.15)", color: dm ? n.neon : "rgba(255,255,255,.8)" }}>{selectedApp.cat}</span><span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full" style={{ background: dm ? n.green : "#4ade80" }} /><span className="text-[9px] font-semibold" style={{ color: dm ? n.green : "#4ade80" }}>Active</span></span></div><button onClick={handleClosePanel} className="w-8 h-8 flex items-center justify-center rounded-xl transition-all" style={{ background: dm ? n.neonFaint : "rgba(255,255,255,.1)", color: dm ? n.textMuted : "rgba(255,255,255,.5)" }}><svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button></div><h2 className="text-xl font-black tracking-tight leading-tight mb-3" style={{ color: dm ? n.textPrimary : "white" }}>{selectedApp.name}</h2><div className="flex gap-0">{["info", "about"].map(tab => (<button key={tab} onClick={() => setMobileTab(tab)} className="flex-1 py-2.5 text-[11px] font-bold uppercase tracking-widest text-center transition-all" style={{ color: mobileTab === tab ? (dm ? n.neon : "white") : (dm ? n.textDim : "rgba(255,255,255,.3)"), borderBottom: mobileTab === tab ? `2px solid ${dm ? n.neon : "white"}` : "2px solid transparent" }}>{tab === "info" ? "Details" : "About"}</button>))}</div></div>
                    <div className="flex-grow hide-scrollbar p-5">{renderPanelContent(selectedApp, mobileTab)}</div>
                    <div className="flex-shrink-0 px-5 pb-5 pt-2" style={{ borderTop: `1px solid ${dm ? n.borderPanel : "rgba(255,255,255,.1)"}` }}><div className="mt-2"><LaunchButton url={selectedApp.url} /></div><p className="text-center text-[8px] font-medium mt-2 truncate" style={{ color: dm ? n.textDim : "rgba(255,255,255,.2)" }}>{selectedApp.url}</p></div>
                  </div>
                )}
              </div>
            </div>
